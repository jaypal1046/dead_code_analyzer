...
List<RegExp> _buildUsagePatterns(String functionName) {
...
bool _isInImportExport(String content, RegExpMatch match) {
  final beforeMatch = content.substring(0, match.start);
  final lines = beforeMatch.split('\n');
  final currentLine = lines.isNotEmpty ? lines.last : '';
...
bool _isInTypeDeclaration(String content, RegExpMatch match) {
  final beforeMatch = content.substring(0, match.start);
  final lines = beforeMatch.split('\n');
  final currentLine = lines.isNotEmpty ? lines.last : '';
...
List<RegExpMatch> _removeDuplicateMatches(List<RegExpMatch> matches) {
...
bool isFunctionCall(String content, RegExpMatch match) {
  ...
  final afterMatch = content.substring(matchEnd).trim();

  // Direct function call with parentheses
  if (afterMatch.startsWith('(')) {
    return true;
  }

  // Function reference (without parentheses) in specific contexts
  final beforeMatch = content.substring(0, match.start);
  final lines = beforeMatch.split('\n');
  final currentLine = lines.isNotEmpty ? lines.last : '';

  // Function reference contexts
  final referenceContexts = [
...
bool isConstructorOrClassName(
  ...
  final constructorPatterns = [
    RegExp(r'\bnew\s+$'), // new Constructor()
    RegExp(r'^\s*(?:final|var|const)\s+\w+\s*=\s*$'), // var x = Constructor()
    RegExp(r':\s*$'), // named parameter: Constructor()
    RegExp(r'return\s+$'), // return Constructor()
    RegExp(r'=>\s*$'), // => Constructor()
    RegExp(r'[\[\(,]\s*$'), // [Constructor()] or (Constructor())
  ];
  ...
  return constructorPatterns.any((pattern) => pattern.hasMatch(currentLine)) ||
      classPatterns.any((pattern) => pattern.hasMatch(currentLine));
}
...
void analyzeFunctionUsagesWithDebug(
  ...
  for (final entry in functions.entries) {
    ...
    if (functionInfo.isPrebuiltFlutterCommentedOut) {
      if (isDebugFunction) print('SKIPPED: Prebuilt Flutter function');
      continue;
    }
    ...
    for (int patternIndex = 0;
      ...
      if (isDebugFunction && matches.isNotEmpty) {
        print('Pattern ${patternIndex + 1} found ${matches.length} matches');
      }
    ...
    if (isDebugFunction) {
      print('Total matches from all patterns: ${allMatches.length}');
    }
    ...
    for (int i = 0; i < allMatches.length; i++) {
      ...
      if (isDebugFunction) {
        ...
        // Show context
        final contextStart = (match.start - 50).clamp(0, content.length);
        final contextEnd = (match.end + 50).clamp(0, content.length);
      ...
      if (!_isValidFunctionUsage(content, match, functionName)) {
        if (isDebugFunction) print('  -> SKIPPED: Invalid usage context');
        continue;
      }

      if (isDebugFunction) print('  -> ✓ VALID function usage!');
      validMatches.add(match);
    }
    ...
    if (isDebugFunction) {
      print('\nFinal results:');
      print('Valid usages (after deduplication): $usageCount');
